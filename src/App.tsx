...
const DEFAULT_FIELD_CORNERS: FieldCornerMap = {
  left: LEFT_FIELD_CORNERS,
  right: RIGHT_FIELD_CORNERS,
};

const DEFAULT_FIELD_GRID_SIZES: FieldGridSizeMap = {
...
const furoSpriteUrls: Record<PlayerDirection, string> = {
...
const playerWalkSprites: Record<PlayerDirection, [string, string, string, string]> = {
  down: [
...
const furoWalkSprites: Record<PlayerDirection, [string, string, string, string]> = {
  down: [
    furoSpriteUrls.down,
...
const FOOTSTEP_SOUNDS: Record<FootstepSound, { label: string; src: string; color: string; playbackRate: number }> = {
...
const WALL_BUMP_SOUNDS: Record<WallBumpSound, { label: string; src: string | null }> = {
...
const IWANA_SPLASH_SOUND_SRC = '/se/Water-splash.mp3';
...
const KURUMI_DEFAULT_SPRITE_W = 96;
const KURUMI_DEFAULT_SPRITE_H = 132;
...
const SpriteItem = ({ z, isSetupMode, onZoneDelete, onZoneClick, isSelected, onSelect, onDragStart, onResizeStart, timeOfDay, seVolume, audioGains }: { key?: string, z: AnimZone, isSetupMode: boolean, onZoneDelete: (id: string) => void, onZoneClick?: (id: string) => void, isSelected: boolean, onSelect: (id: string) => void, onDragStart: (e: React.PointerEvent, id: string) => void, onResizeStart: (e: React.PointerEvent, id: string) => void, timeOfDay: 'morning' | 'day' | 'evening' | 'night', seVolume: number, audioGains: Record<string, number> }) => {
  ...
  const spriteW = z.type === 'kurumi' && zoneSpriteW >= z.w
     ? Math.min(KURUMI_DEFAULT_SPRITE_W, Math.max(8, z.w))
     : zoneSpriteW;
  const spriteH = z.type === 'kurumi' && zoneSpriteH >= z.h
     ? Math.min(KURUMI_DEFAULT_SPRITE_H, Math.max(8, z.h))
     : zoneSpriteH;
  ...
  useEffect(() => {
     ...
     const scheduleJump = () => {
        timeoutId = window.setTimeout(() => {
           if (!active) return;
           let frame = 0;
           const splashAudio = new Audio(IWANA_SPLASH_SOUND_SRC);
           splashAudio.volume = getEffectiveVolume(IWANA_SPLASH_SOUND_SRC, seVolume, audioGains);
...
const AnimationLayer = ({ zones, isSetupMode, onZoneDelete, onZoneClick, selectedZoneId, onSelect, onZoneDragStart, onZoneResizeStart, timeOfDay, seVolume, audioGains }: { zones: AnimZone[], isSetupMode: boolean, onZoneDelete: (id: string) => void, onZoneClick?: (id: string) => void, selectedZoneId: string | null, onSelect: (id: string) => void, onZoneDragStart: (e: React.PointerEvent, id: string) => void, onZoneResizeStart: (e: React.PointerEvent, id: string) => void, timeOfDay: 'morning' | 'day' | 'evening' | 'night', seVolume: number, audioGains: Record<string, number> }) => {
  return (
    <div className="absolute inset-0 pointer-events-none z-20 overflow-hidden">
      ...
      `}</style>
      {zones.map(z => (
         <SpriteItem key={z.id} z={z} isSetupMode={isSetupMode} onZoneDelete={onZoneDelete} onZoneClick={onZoneClick} isSelected={selectedZoneId === z.id} onSelect={onSelect} onDragStart={onZoneDragStart} onResizeStart={onZoneResizeStart} timeOfDay={timeOfDay} seVolume={seVolume} audioGains={audioGains} />
      ))}
    </div>
  );
}
...
const DEBUG_PANEL_WIDTH = 240;
const DIALOG_BOX_DEFAULT_WIDTH = 640;
const DIALOG_BOX_MIN_HEIGHT = 120;
const DIALOG_BOX_MIN_WIDTH = 420;
const DIALOG_BOX_MAX_HEIGHT = 420;
const DEFAULT_SYSTEM_MESSAGE = '「アニメ領域設定」でマップ上に動きを配置し、煙突から煙を出したり池に鳥を浮かべることができます。';
const WATERFALL_BASIN_POINT = { x: 610, y: 560 };
const TAKIURA_WATERFALL_POINT = { x: 514, y: 520 };
const DOUKUTSU_WATERFALL_POINT = { x: 960, y: 540 };
const HOUSE_FIREPLACE_POINT = { x: 950, y: 435 };
const WATERFALL_HEAR_DISTANCE = 900;
const WATERFALL_MIN_GAIN = 0.18;
const WATERFALL_SOUND_SRC = '/se/taki.mp3';
const RIVER_SOUND_SRC = '/se/river.mp3';
const CICADA_SOUND_SRC = '/se/semi.mp3';
const FIREPLACE_SOUND_SRC = '/se/Fireplace.mp3';
const UI_CURSOR_SOUND_SRC = '/se/cursor.mp3';
const UI_FIX_SOUND_SRC = '/se/fix.mp3';
const BATH_CHANGE_SOUND_SRC = '/se/cloak.mp3';
const BATH_SPLASH_SOUND_SRC = '/se/chapon.mp3';
const RIVER_HEAR_DISTANCE = 360;
const FIREPLACE_HEAR_DISTANCE = 520;
const FARM_RIVER_POINTS = [
  { x: 1390, y: 235 },
  { x: 1490, y: 360 },
  { x: 1480, y: 500 },
  { x: 1590, y: 650 },
...
const BGM_FILE_ENTRIES = AUDIO_FILE_ENTRIES.filter(entry => entry.category === 'bgm');

const DEFAULT_MAP_BGM_SOURCES: Record<GameMap, string> = {
...
const DEFAULT_AUDIO_GAINS: Record<string, number> = {
  '/bgm/ie.mp3': 1.8,
...
export default function App() {
  ...
  const renderMenuDetail = (id: MenuItemId) => {
            ...
            <div className="grid grid-cols-2 gap-3">
              {selectedTabItems.map((name, index) => (
                ...
                >
                  ...
                  <span className="text-[#dda15e] text-sm">x{index + 1}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div style={menuPanelBaseStyle} className="grid grid-cols-2 gap-4 h-full">
            {equipmentCharacters.map((char) => (
              <button
                ...
                onClick={() => { playFixSound(); setMenuFocusArea('content'); setMenuContentFocus('primary'); setSelectedEquipmentSlot(`${char.label}-slot1`); }}
                className={`farm-menu-character-stage relative overflow-hidden border cursor-pointer flex flex-col items-center justify-end p-4 ${selectedEquipmentSlot.startsWith(char.label) ? 'is-selected border-white' : 'border-[#5a3010]'}`}
              >
          ...
          <div style={menuPanelBaseStyle} className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {['slot1', 'slot2'].map(slot => {
                  const slotIndex = slot === 'slot1' ? 0 : 1;
                  const slotId = `${selectedEquipmentCharacter.label}-${slot}`;
                  return (
                  <button
                    key={slot}
                    type="button"
                    onPointerDown={() => { setMenuFocusArea('content'); setMenuContentFocus('secondary'); setSelectedEquipmentSlot(slotId); }}
                    onMouseEnter={() => { if (selectedEquipmentSlot !== slotId) playCursorSound(); }}
                    onClick={() => { playFixSound(); setMenuFocusArea('content'); setMenuContentFocus('secondary'); setSelectedEquipmentSlot(slotId); }}
                    className={`text-left rounded px-3 py-2 cursor-pointer ${selectedEquipmentSlot === slotId ? 'bg-[#bc6c25]/45 border border-white' : 'bg-black/35 border border-[#5a3010] hover:bg-[#3a2418]'}`}
                  >
                    <div style={menuTinyLabelStyle}>{slot}</div>
                    <div style={menuValueStyle}>{selectedEquipmentCharacter.slots[slotIndex]}</div>
                  </button>
                )})}
              </div>
    ...
    if (id === 'zukan') {
      return (
        <div className="flex flex-col gap-3 h-full">
          ...
          <div style={menuPanelBaseStyle} className="grid grid-cols-5 gap-2 flex-1">
          ...
          <div className="bg-black/30 rounded p-3 border border-[#5a3010]">
            ...
            <div className="text-[#dda15e] text-sm">フィルタ: {zukanFilter} / シーン回想を選択できます。</div>
          </div>
          <div className="h-5 bg-black/50 border border-[#5a3010] rounded overflow-hidden"><div className="h-full bg-[#bc6c25]" style={{ width: '32%' }} /></div>
        </div>
      );
    }
    ...
    return (
      <div className="grid grid-cols-3 gap-3 h-full">
        ...
        <div style={menuPanelBaseStyle} className="col-span-3 grid grid-cols-3 gap-4">
          {[
  ...
  useEffect(() => {
     movementLockedRef.current = sleepPromptVisible || craftPromptVisible || fishingPromptVisible || kurumiShopOpen || isSleepSequenceActive;
  }, [sleepPromptVisible, craftPromptVisible, fishingPromptVisible, kurumiShopOpen, isSleepSequenceActive]);
  ...
  const resumeCurrentBgm = () => {
    ...
    const currentSource = mapBgmSources[currentMap] ?? DEFAULT_MAP_BGM_SOURCES[currentMap];
  ...
  useEffect(() => {
    ...
    const nextSource = mapBgmSources[currentMap] ?? DEFAULT_MAP_BGM_SOURCES[currentMap];
  ...
  }, [currentMap, bgmVolume, audioGains, mapBgmSources]);
  ...
  useEffect(() => {
      ...
      if (kurumiShopOpen) {
        ...
        if (e.key === 'ArrowUp' || e.key === 'ArrowDown') {
          ...
          setSelectedShopItemIndex(prev => {
            ...
            return (prev + delta + shopItems.length) % shopItems.length;
          });
          return;
        }
      ...
      if (menuOpenRef.current) {
        ...
        if (menuFocusAreaRef.current === 'nav' && e.key === 'ArrowUp') {
          ...
          if (currentMenuItem.id === 'zukan') {
            ...
            setSelectedZukanIndex(moveGridIndex(currentIndex, e.key, 5, 15));
            return;
          }
          ...
          if (currentMenuItem.id === 'equipment') {
            ...
            const currentSlot = selectedEquipmentSlotRef.current.endsWith('slot2') ? 'slot2' : 'slot1';
            if (menuContentFocusRef.current === 'primary') {
            ...
            if (e.key === 'ArrowLeft' && currentSlot === 'slot1') {
              setMenuContentFocus('primary');
              return;
            }
            const nextSlot = e.key === 'ArrowRight' ? 'slot2' : e.key === 'ArrowLeft' ? 'slot1' : currentSlot;
          ...
          if (currentMenuItem.id === 'item') {
            ...
            setSelectedZukanIndex(prev => (prev + moveBy + 6) % 6);
          ...
          if (currentMenuItem.id === 'farm') {
            ...
            setDialogMessage(`${selectedEquipmentSlotRef.current}を選択しました。`);
          } else if (currentMenuItem.id === 'status') {
    ...
    const gameLoop = () => {
      ...
      } else if (!fishingPromptBlockedRef.current && !isInBed && !isInWorkbench) {
        moved = false;
        clickTargetRef.current = null;
        setClickTargetMarker(null);
  ...
  }, [setupMode, bedTiles, workbenchTiles, fishingTiles, sleepPromptVisible, craftPromptVisible, fishingPromptVisible, confirmPromptChoice, activeAutoEventSpot]);
  ...
  const handleMapPointerUp = () => {
     if (setupMode === 'animation' && dragStart && dragCurrent) {
        ...
        if (w > 10 && h > 10) {
           ...
           setZones(prev => [...prev, { id: newId, x, y, w, h, spriteW: w, spriteH: h, type: 'smoke', map: currentMap, timeOfDay }]);
        ...
        <div className="h-[60px] bg-[#2d1b15] border-b-[4px] border-[#bc6c25] flex items-center justify-between px-6 z-40 text-[#fdf6e3]">
          <div className="flex gap-10 text-lg">
            ...
            <span className="flex items-center gap-2 text-[#dda15e]">
              <span className="text-[#fdf6e3] text-sm mt-1">GOLD</span> 500 G
            </span>
          </div>
              ...
              {menuOpen && (
                ...
                >
                  ...
                  >
                    ...
                    <span style={{ color: '#d8c7a5', fontSize: 13 }}>{selectedMenuItem.icon} {selectedMenuItem.label} / [M][Esc]で閉じる</span>
                  </div>
        ...
        {fishingPromptVisible && (
           <div className="absolute inset-0 z-[70] flex items-center justify-center bg-black/20">
              <div className="bg-[#1a100d]/95 border-[4px] border-[#67e8f9] rounded-xl p-6 text-[#fdf6e3] shadow-2xl w-[380px] h-[180px] text-center flex flex-col items-center justify-center">
                 <div className="text-2xl font-bold mb-4">釣りをする？</div>
                 <div className="flex justify-center gap-4">
                    <button
              ...
              <div className="relative flex h-full w-full items-center justify-center px-12 pb-52 pt-12">
                 ...
                 <div className="relative max-h-full max-w-[82%] rounded-xl border-4 border-[#f1c27d] bg-[#1a100d]/95 shadow-2xl overflow-hidden">
                    {activeAutoEventSpot.videoSrc ? (
                       <video
                          src={activeAutoEventSpot.videoSrc}
                       ...
                       <img
                          src={activeAutoEventSpot.imageSrc}
              ...
              >
                 <div className="flex flex-col p-5">
                    <div className="mb-4 flex items-center justify-between border-b border-[#bc6c25]/60 pb-3">
                       ...
                       <button
                          type="button"
                          onClick={() => { playFixSound(); setKurumiShopOpen(false); }}
                                          ...
                                          onChange={(e) => {
                                             ...
                                             setZones(prev => prev.map(z => z.id === selectedZoneId
                                                ? {
                                                   ...z,
                                                   type: val,
                                                   ...(val === 'kurumi' ? { spriteW: KURUMI_DEFAULT_SPRITE_W, spriteH: KURUMI_DEFAULT_SPRITE_H } : {}),
                                                }
                                                : z
                                             ));
                                          }}
                              ...
                              <div className="flex gap-3 items-center bg-[#1a100d]/95 border-[2px] border-[#bc6c25] rounded-lg p-2 text-xs text-[#fdf6e3] z-50">
                                 ...
                                 {isAutoEventSpot && (
                                    <div className="grid grid-cols-2 gap-x-2 gap-y-1 w-[360px]">
                                       {[
                                          ['画像', 'imageSrc', '/img/event.png'],
                                          ['動画', 'videoSrc', '/video/event.mp4'],
                                          ['SE', 'seSrc', '/se/event.mp3'],
                                          ['VOICE', 'voiceSrc', '/voice/voice.mp3'],
